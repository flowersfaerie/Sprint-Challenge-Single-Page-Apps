...
> Answer: Client-side routing allows the client's browser to update the page the displays on their screen without having to completely refresh the page as each page is called from a remote server. Client-side routing allows the client's browser to act as a virtual server which stores a complete copy of the entire website. This allows pages to not only load faster, it also prevents the momentary 'white flash' (page refresh) that is common with server-side routing. 
